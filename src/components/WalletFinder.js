...
import Worker from "worker-loader!../helpers/app.worker";
import {
    Button,
    UncontrolledTooltip,
...
class WalletFinder extends React.Component {
    ...
    search() {
        if (this.state.wallet) {
            if(!confirm("New Vanity Address will hide Previously Generated Address and Seed.\nMake sure you have copied and saved them.\nClick Cancel to go back and copy previous address.")) {
                return;
            }
        }
        ...
        if (typeof(this.worker) === "undefined") {
            ...
            this.worker.onmessage = (event) => {
                if (event.data.action === "finished" && event.data.wallet) {
                    console.log("Generated Address ("+this.props.prefix+"): ", event.data.wallet)
                    this.setState({
    ...
    renderWallet = () => (
        <div className={'cursor-pointer mb-4'} onClick={() => this.walletModal.toggle()}>
            ...
            />
            <h5 className={'responsive-text address-value'}>
        ...
        return (
            <div>
                ...
                <Button outline color="primary" size="sm" onClick={() => this.search()} disabled={this.state.running}>
                    {this.state.running ? "Searching.." : "Search"}
                </Button>
